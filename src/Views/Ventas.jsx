...
const Ventas = () => {
    ...
    const obtenerNombreEmpleado = async (id_empleado) => {
        if (!id_empleado) return '—';
        try {
            const resp = await fetch(`http://localhost:3000/api/empleado/${id_empleado}`);
            if (!resp.ok) return '—';
            const data = await resp.json();
            return `${data.primer_nombre} ${data.primer_apellido}`;
        } catch (error) {
    ...
    const agregarVenta = async () => {
        ...
        try {
            ...
            for (const d of detallesNuevos) {
                await fetch('http://localhost:3000/api/registrardetalles_venta', {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
    ...
    const actualizarVenta = async () => {
        ...
        try {
            ...
            for (const d of actuales) {
                await fetch(`http://localhost:3000/api/eliminardetalles_venta/${d.id_detalle_venta}`, { method: 'DELETE' });
            }
